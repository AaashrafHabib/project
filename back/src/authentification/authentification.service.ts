  ...
  async login(loginDto: LoginDto) {
    ...
    if (role=="Client") {
     ...
     if (client && (await bcrypt.compare(loginDto.password, client.motdepasse))) {
      ...
      const payload = { username: client.username, password: client.motdepasse }; 
    ...
    if (role=="Consultant") {
     ...
     if (consultant && (await bcrypt.compare(loginDto.password, consultant.motdepasse))) {
      ...
      require("dotenv").config();
      const payload = { username: consultant.username, password: consultant.motdepasse }; 
      return {
        access_token: this.jwtService.sign(payload),
      };
    } else {
    ...
    if (role=="Administrator") {
     ...
     if (admin && (await bcrypt.compare(loginDto.password,admin.motdepasse))) {
      require("dotenv").config();
      const payload = { username: admin.username, password: admin.motdepasse }; 
      return {
        access_token: this.jwtService.sign(payload),
      };
    } else {
